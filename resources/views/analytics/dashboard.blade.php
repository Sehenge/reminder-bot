...
<main class="wrap">
    ...
    <section class="grid">
        ...
        <div class="panel wide">
            ...
            <div class="legend"><span><i class="dot"></i>Новые пользователи</span><span><i class="dot green"></i>Активные пользователи</span></div>
            <div class="chart" aria-label="График пользователей за 7 дней">
                @foreach($daily as $day)
                    <div class="day"><div class="columns"><div class="bar" data-value="Новых: {{ $day['users'] }}" style="height:{{ max(2, round($day['users'] / $dailyMax * 100)) }}%"></div><div class="bar green" data-value="Активных: {{ $day['active'] }}" style="height:{{ max(2, round($day['active'] / $dailyMax * 100)) }}%"></div></div><div class="day-label">{{ $day['label'] }}</div></div>
                @endforeach
            </div>
        </div>
